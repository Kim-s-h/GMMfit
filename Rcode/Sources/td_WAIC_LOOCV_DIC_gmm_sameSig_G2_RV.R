...
stat.samp = summary(samps.td2)$statistics

# mean statistics for DIC
beta = matrix(NA, nrow = 2, ncol = 2)
beta[,1] = as.vector(stat.samp[list.par.td2[1:2],1])
beta[,2] = as.vector(stat.samp[list.par.td2[3:4],1])
sig.h = stat.samp[list.par.td2[10],1]
df.h = stat.samp[list.par.td2[11],1]
LS.h = as.numeric(stat.samp[list.LS,1])
LS.h.m = matrix(LS.h, nrow=Nsmp, ncol = 2 )
pi_g = as.vector(stat.samp[list.par.td2[8:9],1])
# mem.est = as.numeric(summary(samps.td2)$quantiles[list.m, 3])
Phi = matrix(NA, nrow=2, ncol =2)
Phi[lower.tri(Phi, diag = T)] = as.vector(stat.samp[list.par.td2[5:7],1])
Phi[1,2] = Phi[2,1]
...
for(s in 1:S){
  loglik[,,s] = matrix(ll.td2[s,], nrow=Nsmp, ncol=J)
}
...
beta.s.elements = samps.td2[[1]][,list.par.td2[1:4]]
Phi.s.elements = samps.td2[[1]][,list.par.td2[5:7]]
...
pi.s = samps.td2[[1]][,list.par.td2[8:9]]
sig.s = as.numeric(samps.td2[[1]][,list.par.td2[10]])
d.s = as.numeric(samps.td2[[1]][,list.par.td2[11]])
