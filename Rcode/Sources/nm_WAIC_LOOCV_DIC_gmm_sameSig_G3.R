...
beta[,1] = as.vector(stat.samp[list.par.nm3[1:2],1])
beta[,2] = as.vector(stat.samp[list.par.nm3[3:4],1])
beta[,3] = as.vector(stat.samp[list.par.nm3[5:6],1])
Phi = matrix(NA, nrow=2, ncol =2)
Phi[lower.tri(Phi, diag = T)] = as.vector(stat.samp[list.par.nm3[7:9],1])
Phi[1,2] = Phi[2,1]
pi_g = as.vector(stat.samp[list.par.nm3[10:12],1])
mem.est = as.numeric(tmp$quantiles[list.m, 3])

# for each iteration
beta.s.elements = samps.nm3[[1]][,list.par.nm3[1:6]]
Phi.s.elements = samps.nm3[[1]][,list.par.nm3[7:9]]
...
pi.s = samps.nm3[[1]][,list.par.nm3[10:12]]
sig.s = samps.nm3[[1]][,list.s]
mem.s = samps.nm3[[1]][,list.m]
  ...
  for(i in 1:Nsmp){
    log.lik.i.s2[s,i] = log(pi.s[s,1]*dmvnorm(y[i,],
      ...
      pi.s[s,2]*dmvnorm(y[i,],
                        ...
                        sigma = Lambda %*% Phi.s[[s]] %*% t(Lambda) + sig.s[s]*diag(J), log=FALSE) +
      pi.s[s,3]*dmvnorm(y[i,],
                        mean = Lambda %*% beta.s[[s]][,3],
                        sigma = Lambda %*% Phi.s[[s]] %*% t(Lambda) + sig.s[s]*diag(J), log=FALSE))
  }
}
