...
stat.samp = summary(samps.de2)$statistics


# mean statistics for DIC
beta = matrix(NA, nrow = 2, ncol = 2)
beta[,1] = as.vector(stat.samp[list.par.de2[1:2],1])
beta[,2] = as.vector(stat.samp[list.par.de2[3:4],1])
tau.m = stat.samp[list.par.de2[10],1]
LS.h = as.numeric(stat.samp[list.LS,1])
LS.h.m = matrix(LS.h, nrow=Nsmp, ncol = 2 )
pi_g = as.vector(stat.samp[list.par.de2[8:9],1])
Phi = matrix(NA, nrow=2, ncol =2)
Phi[lower.tri(Phi, diag = T)] = as.vector(stat.samp[list.par.de2[5:7],1])
Phi[1,2] = Phi[2,1]

# estimates from each iteration
beta.s.elements = samps.de2[[1]][,list.par.de2[1:4]]
Phi.s.elements = samps.de2[[1]][,list.par.de2[5:7]]
...
pi.s = as.data.frame(samps.de2[[1]][,list.par.de2[8:9]])
tau.s = as.numeric(samps.de2[[1]][,list.par.de2[10]])
...
for(s in 1:S){
  loglik[,,s] = matrix(ll.de2[s,], nrow=Nsmp, ncol=J)
}
