# ---------------------------------------------
#  WAIC, LOO-CV, DIC calculation for gmm based on conditional medians
#  G = 3
# ---------------------------------------------
S = Niter
y = resp
...
m.tol = 0.0001   # maximum tol (0.01)
m.eval = 1024L   # The maximum number of function evaluations needed (512L)

stat.samp = summary(samps.de3)$statistics

# mean statistics for DIC
beta = matrix(NA, nrow = 2, ncol = 3)
beta[,1] = as.vector(stat.samp[list.par.de3[1:2],1])
beta[,2] = as.vector(stat.samp[list.par.de3[3:4],1])
beta[,3] = as.vector(stat.samp[list.par.de3[5:6],1])
Phi = matrix(NA, nrow=2, ncol =2)
Phi[lower.tri(Phi, diag = T)] = as.vector(stat.samp[list.par.de3[7:9],1])
Phi[1,2] = Phi[2,1]
tau.m = stat.samp[list.par.de3[13],1]
LS.h = as.numeric(stat.samp[list.LS,1])
LS.h.m = matrix(LS.h, nrow=Nsmp, ncol = 2 )
pi_g = as.vector(stat.samp[list.par.de3[10:12],1])
# mem.est = as.numeric(summary(samps.de3)$quantiles[list.m, 3])

# estimates from each iteration
beta.s.elements = samps.de3[[1]][,list.par.de3[1:6]]
Phi.s.elements = samps.de3[[1]][,list.par.de3[7:9]]
...
pi.s = as.data.frame(samps.de3[[1]][,list.par.de3[10:12]])
tau.s = as.numeric(samps.de3[[1]][,list.par.de3[13]])
...
for(s in 1:S){
  loglik[,,s] = matrix(ll.de3[s,], nrow=Nsmp, ncol=J)
}
...
for(i in 1:Nsmp){
  for(j in 1:J){
    log.lik.h[i,j] = ddexp(as.vector(y[i,j]), 
...
for(s in 1:S){
  log.lik.m.s[s,] = unlist(mclapply(
    ...
    function(a)
      log(pi.s[s,1]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                              Phi = Phi.s[[s]], beta = beta.s[[s]][,1], tau = tau.s[s], resp = a)$integral + 
            pi.s[s,2]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                                Phi = Phi.s[[s]], beta = beta.s[[s]][,2], tau = tau.s[s], resp = a)$integral + 
            pi.s[s,3]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                                Phi = Phi.s[[s]], beta = beta.s[[s]][,3], tau = tau.s[s], resp = a)$integral)
    , mc.cores = numCores))
...
log.lik.m2 = unlist(mclapply(
  lapply(1:Nsmp, function(x) resp[x,]),
  function(a)
    log(pi_g[1]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                          Phi = Phi, beta = beta[,1], tau = tau.m, resp = a)$integral + 
          pi_g[2]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                            Phi = Phi, beta = beta[,2], tau = tau.m, resp = a)$integral + 
          pi_g[3]*hcubature(f_yb, lowerLimit = rep(-Inf,2), upperLimit = rep(Inf,2), vectorInterface = T, tol = m.tol, maxEval = m.eval, 
                            Phi = Phi, beta = beta[,3], tau = tau.m, resp = a)$integral), 
  mc.cores = numCores))
